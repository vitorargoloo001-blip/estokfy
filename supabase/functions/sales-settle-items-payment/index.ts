...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { sale_id, item_allocations, method, paid_at, note } = body;

    if (!sale_id || !Array.isArray(item_allocations) || item_allocations.length === 0 || !method) {
      return json({ error: "payload_invalido", message: "sale_id, item_allocations e method são obrigatórios" }, 400);
    }
    ...
    const { error: idemErr } = await svc.from("idempotency_keys").insert({
      ...
      idem_key: idemKey,
      action: "sales-settle-items-payment",
    ...
    const { data: result, error: rpcErr } = await userClient.rpc("settle_sale_items_payment", {
      p_sale_id: sale_id,
      p_item_allocations: item_allocations,
      p_method: method,
    ...
    if (rpcErr) {
      ...
      if (msg.includes("nenhum_item_selecionado")) return json({ error: "nenhum_item_selecionado", message: "Selecione ao menos um item." }, 400);
      if (msg.includes("item_duplicado_na_requisicao")) return json({ error: "item_duplicado_na_requisicao", message: "Item selecionado duas vezes na mesma requisição." }, 400);
      if (msg.includes("item_nao_pertence_a_venda")) return json({ error: "item_nao_pertence_a_venda", message: "Item não pertence a esta venda." }, 400);
      if (msg.includes("valor_maior_que_saldo_devedor_do_item")) return json({ error: "valor_maior_que_saldo_devedor_do_item", message: "O valor informado é maior que o saldo devedor de um dos itens. Atualize a tela e tente novamente." }, 400);
      if (msg.includes("valor_maior_que_saldo_devedor")) return json({ error: "valor_maior_que_saldo_devedor", message: "O valor informado é maior que o saldo devedor da venda. Atualize a tela e tente novamente." }, 400);
      ...
      console.error("settle items rpc error:", rpcErr);
    ...
    console.error("sales-settle-items-payment error:", e);
    return json({ error: "internal_error", message: "Erro interno. Tente novamente." }, 500);
  }
});
