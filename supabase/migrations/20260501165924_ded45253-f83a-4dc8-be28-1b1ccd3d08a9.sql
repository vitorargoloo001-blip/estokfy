-- Corrige inconsistência entry_type ('in'/'out' → 'income'/'expense') e
-- categorias da edição de venda. Também simplifica regras de fidelidade
-- (recalcula sempre que a venda fica paga, não bloqueia em casos seguros).
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_new_items) LOOP
    ...
    -- Usa 'sale_out' (consistente com create_sale_atomic) e qty negativa
    ...
    VALUES (v_store, v_product_id, 'sale_out', -v_qty, v_unit_cost, 'Edição de venda', 'sale', p_sale_id, v_user);
  ...
  -- Pago -> Pendente: estorna caixa (usa 'expense'/'venda' para aparecer corretamente em relatórios)
  ...
  -- Pendente -> Pago: cria pagamento + entrada de caixa
  IF v_sale.payment_status <> 'paid' AND p_payment_status = 'paid' THEN
    DELETE FROM public.payments WHERE sale_id = p_sale_id;
  ...
  -- Continua paga: ajusta valor/método do pagamento e da entrada existentes
  IF v_sale.payment_status = 'paid' AND p_payment_status = 'paid' THEN
  ...
  -- Fidelidade: só recalcula se nenhum crédito gerado pela venda foi usado
