-- Fix edit_sale_atomic: editing a 'partial' sale incorrectly resets amount_paid to 0
-- and amount_pending to the full net total, making already-settled debts reappear
-- as fully pending in Contas a Receber.
--
-- Root cause: the UPDATE at the end used a binary CASE (paid vs 0) that didn't
-- account for the 'partial' case where real cash payments already exist.
-- Secondary bug: the "non-paid → paid" block deleted all payments (including real
-- partial payments) and created a new payment + cash entry for the FULL amount,
-- double-counting what had already been received.
