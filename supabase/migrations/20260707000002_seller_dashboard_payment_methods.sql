-- =====================================================================
-- Dashboard do vendedor: adiciona contagem de vendas hoje por forma de
-- pagamento (PIX e Dinheiro). Mesma filosofia da migration original
-- (20260703000004): sempre filtrado por created_by = o próprio perfil
-- chamador, nunca expõe dados de outros funcionários ou totais da loja.
-- Conta vendas distintas (não linhas de pagamento) que tiveram ao menos
-- um pagamento no método, hoje.
-- =====================================================================
...
BEGIN
  ...
  SELECT count(DISTINCT s.id) INTO v_today_pix_count
    FROM public.sales s
  ...
  SELECT count(DISTINCT s.id) INTO v_today_cash_count
    FROM public.sales s
    JOIN public.payments p ON p.sale_id = s.id
   WHERE s.store_id = v_store AND s.created_by = v_profile AND s.deleted_at IS NULL
     AND s.status = 'paid' AND s.created_at >= v_today
