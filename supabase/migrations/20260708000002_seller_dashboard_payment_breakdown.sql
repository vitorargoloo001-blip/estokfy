-- =====================================================================
-- Dashboard do vendedor: breakdown genérico por forma de pagamento das
-- vendas de HOJE feitas pelo próprio vendedor (não só PIX/Dinheiro), no
-- mesmo espírito da seção "Formas de pagamento das vendas do período" já
-- existente em Relatórios — mas escopado a hoje + ao vendedor logado.
-- =====================================================================
...
BEGIN
  ...
  SELECT COALESCE(jsonb_object_agg(pm.method, jsonb_build_object('count', pm.cnt, 'amount', pm.amt)), '{}'::jsonb)
    INTO v_today_payment_methods
  FROM (
    SELECT p.method, count(DISTINCT s.id) AS cnt, SUM(p.amount) AS amt
      FROM public.sales s
      JOIN public.payments p ON p.sale_id = s.id
     WHERE s.store_id = v_store AND s.created_by = v_profile AND s.deleted_at IS NULL
       AND s.status = 'paid' AND s.created_at >= v_today
  ...
  RETURN jsonb_build_object(
    ...
    'today_payment_methods', v_today_payment_methods,
