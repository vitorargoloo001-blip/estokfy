-- =========================================================
-- Tabela de auditoria de edições de venda
-- =========================================================
...
CREATE INDEX IF NOT EXISTS sale_audit_logs_sale_idx ON public.sale_audit_logs(sale_id, created_at DESC);
CREATE INDEX IF NOT EXISTS sale_audit_logs_store_idx ON public.sale_audit_logs(store_id, created_at DESC);

ALTER TABLE public.sale_audit_logs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS sale_audit_select ON public.sale_audit_logs;
CREATE POLICY sale_audit_select ON public.sale_audit_logs
  FOR SELECT TO authenticated
  USING (store_id = get_my_store_id());

DROP POLICY IF EXISTS sale_audit_insert ON public.sale_audit_logs;
CREATE POLICY sale_audit_insert ON public.sale_audit_logs
  FOR INSERT TO authenticated
  WITH CHECK (store_id = get_my_store_id());

-- =========================================================
-- RPC transacional para editar venda
-- =========================================================
CREATE OR REPLACE FUNCTION public.edit_sale_atomic(
  ...
  p_notes text,
  p_payment_method text,        -- método "principal" quando paid; ignorado quando pendente/parcial
  p_payment_status text,        -- 'paid' | 'pending' | 'partial'
  p_items jsonb,                -- [{product_id, qty, unit_price}]
  p_allow_negative_stock boolean DEFAULT false,
  p_confirm_revert_payment boolean DEFAULT false
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Apaga movimentos de saída antigos vinculados
  ...
  -- ===== Inserir itens novos, validando estoque =====
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_new_items) LOOP
    ...
    IF v_qty IS NULL OR v_qty <= 0 THEN
      RAISE EXCEPTION 'Quantidade inválida em um item';
    END IF;
    IF v_unit_price < 0 THEN
      RAISE EXCEPTION 'Preço unitário negativo não permitido';
    END IF;
    ...
    IF NOT FOUND THEN
      RAISE EXCEPTION 'Produto inexistente nesta loja';
    END IF;
    ...
    VALUES (v_store, v_product_id, 'out', v_qty, v_unit_cost, 'Edição de venda', 'sale', p_sale_id, v_user);
  ...
  v_old_pm := NULL;
  SELECT method INTO v_old_pm FROM public.payments WHERE sale_id = p_sale_id ORDER BY paid_at LIMIT 1;

  -- Buscar ledger padrão
  ...
  -- Se a venda era PAGA e mudará para pendente/partial -> exige confirmação
  IF v_sale.payment_status = 'paid' AND p_payment_status <> 'paid' THEN
    ...
    -- Estorna caixa: cria contra-lançamento (saída) com referência à venda
    IF v_default_ledger IS NOT NULL THEN
      ...
      VALUES (v_store, v_default_ledger, 'out', 'sale_revert', COALESCE(v_sale.amount_paid, v_sale.net_total),
              v_old_pm, 'Estorno por edição de venda — Motivo: '||p_reason, 'sale', p_sale_id, v_user);
    END IF;

    -- Apaga pagamentos antigos (mantém histórico via sale_audit_logs)
    DELETE FROM public.payments WHERE sale_id = p_sale_id;
  END IF;

  -- Se a venda era pendente/partial e agora vai para PAID
  IF v_sale.payment_status <> 'paid' AND p_payment_status = 'paid' THEN
    -- Garante que existe pagamento cobrindo o total
    DELETE FROM public.payments WHERE sale_id = p_sale_id;
    ...
    IF v_default_ledger IS NOT NULL THEN
      INSERT INTO public.cash_entries (store_id, ledger_id, entry_type, category, amount, payment_method, description, reference_type, reference_id, created_by)
      VALUES (v_store, v_default_ledger, 'in', 'sale', v_net, COALESCE(p_payment_method,'cash'),
              'Recebimento por edição de venda — Motivo: '||p_reason, 'sale', p_sale_id, v_user);
    END IF;
  END IF;

  -- Se continua paga mas mudou método ou valor -> ajusta pagamento existente
  IF v_sale.payment_status = 'paid' AND p_payment_status = 'paid' THEN
    -- Atualiza método/valor do pagamento principal
    UPDATE public.payments SET method = COALESCE(p_payment_method, method), amount = v_net
     WHERE id = (SELECT id FROM public.payments WHERE sale_id = p_sale_id ORDER BY paid_at LIMIT 1);
    -- Sincroniza últimos cash_entries de venda
    -- (não estorna; ajusta valor do lançamento mais recente)
    UPDATE public.cash_entries
       SET amount = v_net,
           payment_method = COALESCE(p_payment_method, payment_method),
           description = COALESCE(description,'') || ' [editado: '||p_reason||']'
     WHERE id = (SELECT id FROM public.cash_entries WHERE reference_type='sale' AND reference_id=p_sale_id AND entry_type='in' ORDER BY occurred_at DESC LIMIT 1);
  END IF;
  ...
  VALUES (
    v_store, p_sale_id, v_profile, v_user, p_reason, v_changes,
    v_before,
...
GRANT EXECUTE ON FUNCTION public.edit_sale_atomic(uuid,text,uuid,timestamptz,numeric,numeric,text,text,text,jsonb,boolean,boolean) TO authenticated;