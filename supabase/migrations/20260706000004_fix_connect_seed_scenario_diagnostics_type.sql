-- =====================================================================
-- Fix: connect_seed_scenario_completo declarava v_match_id como UUID mas
-- usava `GET DIAGNOSTICS v_match_id = ROW_COUNT` (um inteiro) para capturar
-- a contagem de matches pendentes criados — erro pré-existente desde a
-- migration original (20260620000009), nunca exercitado antes porque a
-- função sempre falhava antes de chegar lá (account_number/account_type).
-- Troca por uma variável INT dedicada e soma ao total de matches criados.
-- =====================================================================
...
BEGIN
  ...
  INSERT INTO public.bank_connections (id, store_id, bank_name, bank_code, account_number, account_type, account_holder, status, is_active, last_sync_at)
  VALUES (gen_random_uuid(), p_store_id, 'Banco PIX Sandbox', '077', '****1001', 'checking', 'LOJA DEMO LTDA', 'connected', true, now())
  RETURNING id INTO v_conn_pix;

  INSERT INTO public.bank_connections (id, store_id, bank_name, bank_code, account_number, account_type, account_holder, status, is_active, last_sync_at)
  VALUES (gen_random_uuid(), p_store_id, 'Cartão Sandbox', '341', '****1002', 'checking', 'LOJA DEMO LTDA', 'connected', true, now())
  RETURNING id INTO v_conn_card;

  INSERT INTO public.bank_connections (id, store_id, bank_name, bank_code, account_number, account_type, account_holder, status, is_active, last_sync_at)
  VALUES (gen_random_uuid(), p_store_id, 'Dinheiro/Boleto Sandbox', '001', '****1003', 'checking', 'LOJA DEMO LTDA', 'connected', true, now())
  ...
  IF array_length(v_sale_ids, 1) >= 6 THEN
    ...
    GET DIAGNOSTICS v_pending_matches = ROW_COUNT;
    v_matches_created := v_matches_created + v_pending_matches;
  END IF;
