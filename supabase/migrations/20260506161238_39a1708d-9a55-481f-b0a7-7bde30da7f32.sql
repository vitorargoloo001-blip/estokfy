...
DROP FUNCTION IF EXISTS public.create_sale_atomic(uuid, uuid, jsonb, jsonb, jsonb, numeric, date, timestamptz);
...
begin
  ...
  insert into public.sales(id, store_id, customer_id, status, discount_total, created_by, due_date, created_at, notes)
  values (v_sale_id, p_store_id, p_customer_id, 'paid', coalesce(p_discount,0), v_ctx.profile_id, p_due_date, v_op_date, v_notes);
...
CREATE INDEX IF NOT EXISTS idx_sales_notes_trgm
  ON public.sales USING gin (notes public.gin_trgm_ops)
