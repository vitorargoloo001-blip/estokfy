-- =====================================================================
-- Fix: delete_sale_permanently somava TODOS os payments (inclusive
-- 'pending' e, agora, 'credit') em v_total_paid para gerar o estorno de
-- caixa (cash_entries expense). Isso já estornaria dinheiro que nunca
-- entrou (parte pendente) e, com o novo método 'credit', estornaria
-- "caixa" para um consumo de crédito que nunca gerou cash_entries de
-- entrada — um estorno fantasma. v_total_paid agora exclui os métodos
-- que não representam entrada real de caixa.
-- =====================================================================
...
BEGIN
  ...
  -- v_total_paid: só métodos que representam entrada REAL de caixa.
  -- 'pending' nunca entrou; 'credit' consome saldo do cliente, não caixa.
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
            'id', p.id, 'method', p.method, 'amount', p.amount, 'paid_at', p.paid_at)), '[]'::jsonb),
         COALESCE(SUM(p.amount) FILTER (WHERE p.method NOT IN ('pending', 'credit')), 0)
  ...
  -- 2) Cash reversal if there were real cash payments
  ...
  -- 3) Loyalty: revert credit uses for this sale (também cobre o novo
  -- método 'credit', que grava em loyalty_credit_uses do mesmo jeito)
  BEGIN
